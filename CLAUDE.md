...
### Phase 12: Community Pages v2.0 Redesign (12-01)
- **Events page** (`src/app/events/page.tsx`): Added InteriorHero with "Events" heading, SectionLabel "WHAT'S ON", bg-surface-page background, kept all existing filter logic (useState/useMemo, OR within dimension, AND across)
- **EventCard** (`src/components/events/EventCard.tsx`): Redesigned to white rounded-2xl cards on bg-surface-card, navy date badge (rounded-xl bg-primary-900), blue accent tags (bg-accent-100), format tags with border-divider, CircleArrow indicator, hover lift+shadow
- **EventFilters** (`src/components/events/EventFilters.tsx`): Rounded pills with blue active states (bg-accent-100 text-accent-600 when active, border-divider when inactive), bg-surface-card container, blue focus rings
- **EventGrid** (`src/components/events/EventGrid.tsx`): Updated to rounded-2xl empty state card on bg-surface-card
- **Event detail** (`src/app/events/[slug]/page.tsx`): bg-surface-page background, rounded-2xl metadata card on bg-surface-card, blue accent tags, audience badges with border-divider, CircleArrowCTA for "Register Your Interest", preserved JSON-LD structured data
- **Who We Serve** (`src/app/who-we-serve/page.tsx`): InteriorHero "Who We Serve", SectionLabel "OUR COMMUNITIES" center + EmphasisHeading "We're Here for *You*", audience segments with divider lines instead of alternating backgrounds, dark navy CTA with EmphasisHeading + CircleArrowCTA light variant
- **AudienceCard** (`src/components/services/AudienceCard.tsx`): Updated to rounded-2xl bg-surface-card for access points and testimonial cards, CircleArrowCTA replaces Button, testimonial in card with border-l accent
- **Get Involved** (`src/app/get-involved/page.tsx`): InteriorHero "Get Involved", SectionLabel "MAKE A DIFFERENCE" + EmphasisHeading, involvement cards as rounded-2xl bg-surface-card with 3-column grid (icon + content + CTA), CircleArrowCTA per card, dark navy CTA banner
- **Partnerships** (`src/app/partnerships/page.tsx`): InteriorHero "Partnerships", SectionLabel "OUR APPROACH" + EmphasisHeading "*Stronger* Together", approach list in rounded-2xl card, partner grid in 2x4 rounded-2xl cards with hover lift, SectionLabel "OUR PARTNERS" + EmphasisHeading "*Trusted* By", dark navy CTA with CircleArrowCTA light variant
- **Team** (`src/app/team/page.tsx`): InteriorHero "Our Team", SectionLabel "LEADERSHIP" + EmphasisHeading "Board of *Directors*", 4-column frosted glass portrait cards (3:4 aspect ratio), divider between sections, SectionLabel "OUR PEOPLE" + EmphasisHeading "Our *Staff*", 3-column grid
- **TeamMemberCard** (`src/components/team/TeamMemberCard.tsx`): Frosted glass portrait cards with gradient placeholder bg, dark gradient from bottom, backdrop-blur-md bg-white/10 overlay at bottom, name in white serif, role in accent-300, bio text truncated with line-clamp-3, gradientIndex prop for visual variety
- **TeamGrid** (`src/components/team/TeamGrid.tsx`): Updated to pass gradientIndex to cards
- **Testimonials** (`src/app/testimonials/page.tsx`): InteriorHero "Testimonials", SectionLabel "TESTIMONIALS" center + EmphasisHeading "What *Our Community* Says", 4-column portrait cards (3:4 aspect ratio) with gradient placeholders, dark gradient from bottom, segment tag pills (bg-accent-500), truncated quotes, decorative Play button icon (backdrop-blur-sm), dark navy CTA with EmphasisHeading + CircleArrowCTA
- **Placeholder image** (`public/images/placeholder-hero.svg`): Dark navy gradient SVG (primary-950 to primary-800) for InteriorHero backgrounds across all 6 community pages
