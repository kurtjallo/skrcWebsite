...
- **InsetHero** (`src/components/layout/InsetHero.tsx`): Rounded-corner container (rounded-[20px], px-4 pt-4), optional background image with next/image or gradient placeholder fallback, dark overlay gradient, dot texture overlay, content positioned at bottom
- **InteriorHero** (`src/components/layout/InteriorHero.tsx`): Shorter 45vh hero wrapping InsetHero, centered EmphasisHeading h1, supports optional backgroundImage
- **PreFooterCTA** (`src/components/shared/PreFooterCTA.tsx`): Large image/gradient card with curved clip-path at top, EmphasisHeading h2, body text, CircleArrowCTA light variant, min-h-[500px]

### Phase 10: Homepage Redesign (10-01)
- Homepage components redesigned with v2.0 design language (documented in main repo)

### Phase 11: Core Page Redesigns (11-01)
- **Design tokens updated** (`src/app/globals.css`): Migrated from antique gold accent to royal blue (`#3b3bff`), deep navy primary (`#1a1a5e`), blue-grey surfaces (`surface-page: #f0f1f8`, `surface-card: #ffffff`, `surface-dark: #121240`), divider token (`#e0e2ea`), updated text colors
- **Services overview** (`src/app/services/page.tsx`): Complete rewrite -- InteriorHero "Our *Services*", SectionLabel "OUR SERVICES" + EmphasisHeading "Solutions We *Provide*", 4 stacked vertical pillar cards with two-column layout (image placeholder left ~40%, content right ~60%), alternating row direction, image cards have gradient bg + dot texture + icon badge + service title + CircleArrowCTA light, content has orgName overline + "Why It Matters:" serif label + description + "What's Included:" serif label + CheckCircle blue checkmark list, PreFooterCTA at bottom
- **Service detail pages** (`src/components/services/PillarDetail.tsx`): Complete rewrite -- InteriorHero with pillar name, back link to /services, two-column white cards (Who is this for? with icon badge + SectionLabel / What to expect), services list in white card with SectionLabel + EmphasisHeading + bordered items with CheckCircle, testimonial in rounded dark gradient card with dot texture, Ready to Get Started CTA with CircleArrowCTA, PreFooterCTA linking back to all services
- **About page** (`src/app/about/page.tsx`): Complete rewrite -- InteriorHero "About *Us*", asymmetric two-column intro (narrow left with SectionLabel + "Who We Are" h2, wide right with body paragraphs + EmphasisHeading closing line with accentItalic), mission section two-column (gradient placeholder image left + white card right with SectionLabel "OUR MISSION" + EmphasisHeading "Driven by *Purpose*" accentItalic + CircleArrowCTA), vision section (centered SectionLabel + large EmphasisHeading statement + panoramic gradient image + 4 stat cards with blue left border), values section (centered SectionLabel + EmphasisHeading + numbered white cards with hover lift), PreFooterCTA
- **Contact page** (`src/app/contact/page.tsx`): Complete rewrite -- full dark page design (bg-primary-950), NOT using InteriorHero, dot texture + gradient wash overlays, accent-400 overline + EmphasisHeading "Let's Start a *Conversation*" in white, thin white/10 separator, two-column layout (ContactInfo left + ContactForm right), all text styled for dark bg
- **ContactInfo** (`src/components/contact/ContactInfo.tsx`): Added `variant` prop ("light" | "dark"), dark mode: white text, bg-white/10 icon circles, accent-400 labels, border-white/10 confidentiality card, bg-white/5 card backgrounds
- **ContactForm** (`src/components/contact/ContactForm.tsx`): Added `variant` prop ("light" | "dark"), dark mode: bg-white/5 inputs with border-white/15, white/80 labels, white placeholder/30, white submit button on primary-950, rounded-xl corners, white chevron SVG for select, focus:ring-offset-primary-950
